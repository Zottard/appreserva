            ...
            <template #summary>
              <span class="flex items-center gap-3">
                ...
                <span class="flex flex-col text-left">
                  <span class="text-morado text-base font-bold">{{ fullName(pasajero) }}</span>
                  <span v-if="pasajero.genero" class="text-gris text-xs font-medium">{{ pasajero.genero }}</span>
                </span>
              </span>
            </template>

            <dl class="w-full grid grid-cols-2 gap-x-4 gap-y-3 pl-13">
              <div v-if="pasajero.nro_pasaporte" class="flex flex-col">
                <dt class="text-gris text-xs font-medium">Pasaporte</dt>
                <dd class="font-inter text-morado text-sm font-medium">{{ pasajero.nro_pasaporte }}</dd>
              </div>
              <div v-if="pasajero.fecha_nacimiento" class="flex flex-col">
                <dt class="text-gris text-xs font-medium">Nacimiento</dt>
                <dd class="font-inter text-morado text-sm font-medium">{{ formatFecha(pasajero.fecha_nacimiento) }}</dd>
              </div>
              <div v-if="pasajero.nacionalidad" class="flex flex-col">
                <dt class="text-gris text-xs font-medium">Nacionalidad</dt>
                <dd class="text-morado text-sm font-semibold">{{ pasajero.nacionalidad }}</dd>
              </div>
              <div v-if="pasajero.genero" class="flex flex-col">
                <dt class="text-gris text-xs font-medium">Género</dt>
                <dd class="text-morado text-sm font-semibold">{{ pasajero.genero }}</dd>
              </div>
            </dl>
          </Accordion>
        </li>
